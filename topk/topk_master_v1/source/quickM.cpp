...
quickM::quickM(load_graph& G, double used_gamma) {
	...
	this->max_clique_start_time = std::chrono::steady_clock::now();
	this->max_clique_size = 0;
	this->max_clique_time = 0.0;
	this->max_clique_vertices.clear();
	this->runtime_limit_start_time = this->max_clique_start_time;
	this->max_runtime_seconds = 0.0;
	this->runtime_limited = false;
...
void quickM::get_quasi_clique() {
	...
	int cur_size = (int)curQ.size();
	this->all_quasi_cliques.insert(make_pair(-cur_size, curQ));
	...
	if (cur_size > max_clique_size)
	{
		...
		max_clique_time = std::chrono::duration<double>(std::chrono::steady_clock::now() - max_clique_start_time).count();
		max_clique_vertices.assign(curQ.begin(), curQ.end());
	}
}

void quickM::reset_max_clique_tracking()
{
	max_clique_start_time = std::chrono::steady_clock::now();
	max_clique_size = 0;
	max_clique_time = 0.0;
	max_clique_vertices.clear();
}

int quickM::get_max_clique_size() const
{
	return max_clique_size;
}

double quickM::get_max_clique_time() const
{
	return max_clique_time;
}

const std::vector<int> &quickM::get_max_clique_vertices() const
{
	return max_clique_vertices;
}

void quickM::set_runtime_limit(std::chrono::steady_clock::time_point start_time, double runtime_seconds)
{
	runtime_limit_start_time = start_time;
	max_runtime_seconds = runtime_seconds;
	runtime_limited = runtime_seconds > 0.0;
	timed_out = false;
}

bool quickM::has_timed_out() const
{
	return timed_out;
}

bool quickM::time_limit_exceeded()
{
	if (!runtime_limited)
		return false;
	if (timed_out)
		return true;
	double elapsed = std::chrono::duration<double>(std::chrono::steady_clock::now() - runtime_limit_start_time).count();
...
void quickM::back_track() {
	...
	if (indeg_X[v] + exdeg_X[v] >= min_degree) {
		...
		if (isPossibleToExtendX == true && !time_limit_exceeded())
			back_track();
...
void quickM::extract_all_quasi_cliques() {
	...
	for (int start_node = 0; start_node < this->V && !time_limit_exceeded(); start_node++) {
		...
		if (chk == true && !time_limit_exceeded()) {
