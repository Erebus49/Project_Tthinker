...
void load_graph::start(const char* input_path, const char* output_path, double gamma_value, double gamma_prime_value, int k_value, int k_prime_value, int min_size_value) {
	snprintf(input_address, sizeof(input_address), "%s", input_path);
	snprintf(output_address, sizeof(output_address), "%s", output_path);
	gamma = gamma_value;
	gamma_prime = gamma_prime_value;
	k = k_value;
	k_prime = k_prime_value;
...
void load_graph::read_graph() {
	...
	while (std::getline(std::cin, s)) {
		...
		if (((int)vec_str.size()) >= 4 && vec_str[0] == "p" && this->all_num(vec_str[2])) {
			int declared_vertices = to_int(vec_str[2]);
			for (int vertex = 1; vertex <= declared_vertices; vertex++) {
				int original_vertex = vertex;
				this->add_vertex(original_vertex);
			}
		}
		else if (((int)vec_str.size()) >= 2) {
			int first_endpoint = 0;
			if (vec_str[0] == "e")
				first_endpoint = 1;
			if (((int)vec_str.size()) >= first_endpoint + 2) {
				bool is_all_num = true;
				for (int i = first_endpoint; i < first_endpoint + 2; i++) is_all_num &= this->all_num(vec_str[i]);
				if (is_all_num) {
					int A = to_int(vec_str[first_endpoint]), B = to_int(vec_str[first_endpoint + 1]);
					// ignore a self loop
					if (A == B)
						continue;
