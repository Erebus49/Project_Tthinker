...
public:
    ...
    void start(int thread_id)
    {
        ...
        string output_path = build_thread_output_file_path(thread_id);
        gfpout = fopen(output_path.c_str(), "wt");
        if (gfpout == NULL)
            throw runtime_error(string("Error: cannot open output file for write: ") + output_path);
        ...
        if (task == NULL) 
        {
            ...
            while (!collector.empty())
            {
                ...
                if (runtime_limit_reached())
                {
                    ...
                    while (!collector.empty())
                    {
                        TaskT *remaining = collector.front();
                        collector.pop();
                        delete remaining;
                    }
                    return false;
    ...
    void run()
    {
        ...
        if (thread_max_clique_size > 0 && gfpout != NULL)
        {
            fprintf(gfpout, "%d %.6f", thread_max_clique_size, thread_max_clique_time);
            for (size_t i = 0; i < thread_max_clique_vertices.size(); i++)
                fprintf(gfpout, " %d", thread_max_clique_vertices[i]);
            fprintf(gfpout, "\n");
        }
    }
};
