...
std::chrono::steady_clock::time_point global_program_start_time;
...
thread_local vector<int> thread_max_clique_vertices;
...
inline void mark_thread_output_start_time()
{
	...
	thread_max_clique_size = 0;
	thread_max_clique_time = 0.0;
	thread_max_clique_vertices.clear();
}
...
inline bool runtime_limit_reached()
{
	if (MAX_RUNTIME_SEC <= 0.0)
		return false;
	double elapsed = std::chrono::duration<double>(std::chrono::steady_clock::now() - global_program_start_time).count();
	return elapsed >= MAX_RUNTIME_SEC;
}

inline void request_global_stop()
{
