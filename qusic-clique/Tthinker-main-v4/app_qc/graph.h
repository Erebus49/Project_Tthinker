...
{
	double elapsed_sec = get_thread_output_elapsed_sec();
	if (nclique_size > thread_max_clique_size)
	{
		thread_max_clique_size = nclique_size;
		thread_max_clique_time = elapsed_sec;
		thread_max_clique_vertices.assign(nclique_size, 0);
		for (int i = 0; i < nclique_size; i++)
			thread_max_clique_vertices[i] = index2id[pclique[i].nvertex_no];
	}
}
