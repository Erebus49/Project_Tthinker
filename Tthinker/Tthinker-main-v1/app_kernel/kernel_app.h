...
public:
    ...
    virtual void compute(ContextT& context)
    {
        ...
        int thread_lb = Expand(context.pvertices, context.nclique_size, context.num_of_cands, context.num_of_tail_vertices, gfpout, split_g);
        record_thread_lb(thread_lb);
...
public:
    ...
    virtual void load_data(char* file_path)
    {
        ...
        if (global_pvertices != NULL) {
            ...
            int nlen;
            int spawn_count = 0;
            while (spawn_count < prime_k && fscanf(fp, "%d", &nlen) == 1) {
                if (nlen <= 0)
                    continue;
                // read kernel
