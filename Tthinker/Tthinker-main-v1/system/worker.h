...
public:
    ...
    Worker(int comper_num) : big_files_seq(comper_num, 1), reg_files_seq(comper_num, 1), big_files_names(comper_num), reg_files_names(comper_num)
    {
        ...
        string buffer_root = get_buffer_root_dir();
        recursive_mkdir(buffer_root.c_str());
        cleanup_stale_buffer_runs(buffer_root);

        auto now_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
                          std::chrono::system_clock::now().time_since_epoch())
                          .count();
        TASK_DISK_BUFFER_DIR = buffer_root + "/run_" + to_string((long long)getpid()) + "_" + to_string((long long)now_ms);
        register_buffer_cleanup_hook_once();
        recursive_mkdir(TASK_DISK_BUFFER_DIR.c_str());
        start_detached_buffer_cleaner(TASK_DISK_BUFFER_DIR);
        ...
        // Initialize summary output early so timeout during load_data still leaves a file.
        init_output_summary(THREAD_OUTPUT_DIR, num_compers);
    }

    virtual ~Worker()
    {
        cleanup_buffered_task_files();
    ...
    void cleanup_buffered_task_files()
    {
        string file;
        while (global_Lbig.dequeue(file))
        {
            remove_file_if_exists(file);
            if (global_Lbig_num > 0)
                global_Lbig_num--;
        }

        while (global_Lreg.dequeue(file))
        {
            remove_file_if_exists(file);
            if (global_Lreg_num > 0)
                global_Lreg_num--;
        }
